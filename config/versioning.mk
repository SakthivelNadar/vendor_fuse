...
ifeq ($(FUSE_BUILD_VARIANT),OFFICIAL)
    ...
    PROD_VERSION += fuse_$(TARGET_DEVICE)-$(PLATFORM_FUSE_RELEASE)-$(shell date +%m%d%H%M)-OFFICIAL
else
    PLATFORM_FUSE_VERSION := $(PLATFORM_FUSE_RELEASE)
    PROD_VERSION += fuse_$(TARGET_DEVICE)-$(PLATFORM_FUSE_RELEASE)-$(shell date +%m%d%H%M)-UNOFFICIAL
endif
